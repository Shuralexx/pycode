...
class ad9643:
    ...
    def ad9643_write_reg(self, addr, para):
        ...
        if rx and len(rx) == 7:
            ...
            if rx_list == command_Rev:
                ...
                data = self.ad9643_read_reg(addr)
                self.config_current[self.regaddr.index(addr)] = data
            else:
                print('wrong received command')
        else:
            print('data wrong')
    ...
    def ad9643_read_all_regs(self):
        """
        读取所有寄存器的值
        :return: 寄存器地址和对应值的字典
        """
        reg_values = {}
        for addr in self.regaddr:
            if addr == 0x02:  # CHIPGRADE寄存器不读取
                continue
            value = self.ad9643_read_reg(addr)
    ...
    def udp_Connect(self, local_ip_str, remote_ip_str):
        ...
        print(f"本地IP配置成功: {local_ok}, 回包: {rx}")
        while self.myserial.ser.in_waiting > 0:
            self.myserial.ser.read()
        ...
        print(f"远端IP配置成功: {remote_ok}, 回包: {rx2}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.device = ad9643()
        self.setup_ui()
        self.modified_regs = set()  # 用于跟踪修改过的寄存器
    ...
    def setup_ui(self):
        ...
        self.read_all_btn = QPushButton("读全部寄存器")
        self.read_all_btn.clicked.connect(self.read_all_regs)
        rw_layout.addWidget(self.write_btn)
        rw_layout.addWidget(self.read_btn)
        rw_layout.addWidget(self.read_all_btn)
        ...
        main_layout.addWidget(self.text_edit, 3, 0, 1, 2)  # 放在底部横跨全行
        ...
        # --------6. 图形显示Tab（四个） --------
    # graph_diaplay是主显示区Tab控件，包含四个页面

        self.graph_diaplay = QTabWidget()  # 主tab控件
        ...
        self.tab1_measure.horizontalHeader().setStyleSheet('QHeaderView::section{background:grey;color:white;font:bold}')
        self.tab1_measure.setFixedWidth(700)
        # 实时波形pyqtgraph图表
        self.tab1_plt = pg.PlotWidget()
        self.tab1_plt.showGrid(x=True, y=False)
        ...
        self.tab2_plt = pg.PlotWidget()
        self.tab2_plt.showGrid(x=False, y=True)
        ...
        self.tab2_measure.horizontalHeader().setStyleSheet('QHeaderView::section{background:grey;color:white;font:bold}')
        ...
        self.tab3_plt = pg.PlotWidget()
        self.tab3_plt.showGrid(x=False, y=True)
        ...
        self.tab3_measure.horizontalHeader().setStyleSheet('QHeaderView::section{background:grey;color:white;font:bold}')
        ...
        self.tab4_table = QTableWidget()
        self.tab4_table.setColumnCount(5)
        self.tab4_table.setHorizontalHeaderLabels(['Register Name', 'Address', 'Value', 'Default', 'Mode'])
        self.tab4_table.horizontalHeader().setStyleSheet('QHeaderView::section{background:grey;color:white;font:bold}')
        ...
        self.refresh_reg_table()  # 初始化表格内容
        self.tab4_table.cellChanged.connect(self.reg_cell_changed)
        self.graphTab4_Layout.addWidget(self.tab4_Label, 1, 1, 1, 6)
        self.graphTab4_Layout.addWidget(self.tab4_table, 2, 1, 6, 6)

        # 将四个页面加入Tab控件
        self.graph_diaplay.addTab(self.graphTab1, 'Time Domain Display')           # 时域波形
        self.graph_diaplay.addTab(self.graphTab2, 'Sample Histogram Analysis')     # 直方图
        self.graph_diaplay.addTab(self.graphTab3, 'Spectral Analysis')             # 频谱图
        self.graph_diaplay.addTab(self.graphTab4, 'Register Config')               # 寄存器配置

        # 把Tab控件添加到主布局
        main_layout.addWidget(self.graph_diaplay, 1, 2, 3, 2)  # 放在第1行第2列（row=1, column=2），占3行2列
    ...
    def write_all_regs(self):
        if not self.modified_regs:
            QMessageBox.information(self, "批量写入", "没有被修改过的寄存器，无需写入。")
            return
        # 先全部检查一遍，确保每一行都是合法16进制或为空或None
        for i in self.modified_regs:
        ...
        for row in self.modified_regs:
            ...
            cell_item = self.tab4_table.item(row, 2)
            if cell_item is None:
                continue
            cell_str = cell_item.text().replace("0x", "").strip()
            if not cell_str or cell_str.lower() == "none":
            ...
            self.device.ad9643_write_reg(addr, val)
            count += 1
        self.append_text(f"只写入被修改的寄存器，共写入{count}个。")
        self.modified_regs.clear()  # 写完清空
        self.refresh_reg_table()
    ...
    def read_reg(self):
        ...
        try:
            ...
            if val is not None:
                self.device.config_current[self.device.regaddr.index(addr)] = val
    ...
    def read_all_regs(self):
        def refresh_table():
            try:
                reg_values = self.device.ad9643_read_all_regs()
                if reg_values:
                    self.append_text("读取所有寄存器成功：")
                    for addr, value in reg_values.items():
                        self.append_text(f"寄存器0x{addr:02X}，值=0x{value:02X}")
                    self.refresh_reg_table()  # 刷新表格显示
                else:
                    self.append_text("未读取到任何寄存器值。")
            except Exception as e:
                self.append_text(f"读取所有寄存器失败: {e}")
        thread = threading.Thread(target=refresh_table, daemon=True)
        thread.start()
    ...
    def refresh_reg_table(self):
        ...
        for i, (name, addr, val, dft, mode) in enumerate(zip(
            ...
            # 列0: 寄存器名
            item0 = QTableWidgetItem(str(name))
            item0.setFlags(item0.flags() & ~Qt.ItemIsEditable)  # 只读
            self.tab4_table.setItem(i, 0, item0)
            # 列1: 地址
            item1 = QTableWidgetItem(f"0x{addr:02X}")
            item1.setFlags(item1.flags() & ~Qt.ItemIsEditable)  # 只读
            self.tab4_table.setItem(i, 1, item1)
            # 列2: 当前值（仅此列允许编辑，若mode=='R'则不可编辑）
            vitem = QTableWidgetItem("" if val is None else f"0x{val:02X}")
            if mode == 'R':
                vitem.setFlags(vitem.flags() & ~Qt.ItemIsEditable)  # 只读寄存器不可编辑
            self.tab4_table.setItem(i, 2, vitem)
            # 列3: 默认值
            item3 = QTableWidgetItem("" if dft is None else f"0x{dft:02X}")
            item3.setFlags(item3.flags() & ~Qt.ItemIsEditable)  # 只读
            self.tab4_table.setItem(i, 3, item3)
            # 列4: R/W
            item4 = QTableWidgetItem(mode)
            item4.setFlags(item4.flags() & ~Qt.ItemIsEditable)  # 只读
            self.tab4_table.setItem(i, 4, item4)
    ...
    def reg_cell_changed(self, row, col):
        ...
        try:
            ...
            self.modified_regs.add(row) # 标记此行被改过
